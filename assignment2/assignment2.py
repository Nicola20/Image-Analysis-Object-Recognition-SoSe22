...
def cornerness(M):
    ...
    if np.linalg.det(M) == 0 or np.trace(M) == 0:
        return 0
    else:
        return np.linalg.det(M) / np.trace(M)
...
def roundness(M):
    ...
    if np.linalg.det(M) == 0 or np.trace(M) == 0:
        ...
        return (4 * np.linalg.det(M)) / (np.trace(M)**2)
...
def task_2(img_name, norm, gradient_x, gradient_y):
...
def main():
    ...
    task_2(img_name_1, norm_1, gradient_x_1, gradient_y_1)
    # task_2(img_name_2, norm_2, gradient_x_2, gradient_y_2)
    # task_2(img_name_3, norm_3, gradient_x_3, gradient_y_3)
